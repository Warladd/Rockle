...
@export var structure_bar_container : VBoxContainer
...
# Control
@export var last_structure_timer : Timer
var last_structure : String = "Ball"
...
var testing_control : bool = false
...
func _physics_process(delta) -> void:
	if last_structure != "Ball" and last_structure_timer.is_stopped():
		last_structure_timer.start()
		print("resetting last structure")
	if testing_control:
		if !disky and !cubey and !pillary and !bally and !wally:
			return
...
func _input(event) -> void:
	...
	if event.is_action_pressed("down"):
		last_structure = "Wall"
		print("wall")
	if testing_control and event.is_action_pressed("spawn") and !structure_loading:
		structure_loading = true
		print("awaiting spawn")
		if last_structure == "Ball":
			await get_tree().create_timer(0.1).timeout
		if last_structure == "Pillar":
			pillary = true
			structure_loading = false
			print("spawning pillar")
		elif last_structure == "Cube":
			cubey = true
			structure_loading = false
			print("spawning")
		elif last_structure == "Disk":
			disky = true
			structure_loading = false
		elif last_structure == "Wall":
			wally = true
			structure_loading = false
		else:
			bally = true
...
func _on_control_button_pressed() -> void:
	...
	if testing_control:
		disk_timer.paused = true
		pillar_timer.paused = true
		ball_timer.paused = true
		wall_timer.paused = true
		cube_timer.paused = true
		structure_bar_container.hide()
	elif !testing_control:
		disk_timer.paused = false
		pillar_timer.paused = false
		ball_timer.paused = false
		wall_timer.paused = false
		cube_timer.paused = false
		structure_bar_container.show()


func _on_last_structure_timer_timeout() -> void:
	last_structure = "Ball"
	print("ball")
